...
namespace Base.UI.Mobile
{
   ...
   public class Control : IOnDeserialization
   {
      ...
      [NonSerialized]
      public TouchCollection oldTouchCollection;
      [NonSerialized]
      ...
      public Control(string name, string value, EngineRectangle bounds, Color c)
      {
         SetDefaults();
         Name = name;
         this.value = value;
         this.bounds = bounds;
         textColorUint = c.PackedValue;
         textColor = c;
      }
      ...
      public void SetDefaults()
      {
         ...
         imageReference.Add(Enums.cState.draging.ToString(), "defaultTexture");
      ...
      public void setImageReferences(string none, string hover, string pressed, string released)
      {
         ...
         imageReference.Add(Enums.cState.draging.ToString(), pressed);
      ...
      public virtual void Update(int dt)
      {
         if (isEnabled)
         {
            TouchCollection newTouchCollection = TouchPanel.GetState();
            if (newTouchCollection.Count > 0)
            {
               TouchLocation TL = newTouchCollection[0];
               if (clickState == Enums.cState.draging)
               {
                  Drag(oldTouchCollection[0], TL);
               }
               else if (TL.Position.X > bounds.X && TL.Position.X < bounds.X + bounds.Width &&
                        TL.Position.Y > bounds.Y && TL.Position.Y < bounds.Y + bounds.Height)
               {
                  if (!isFocused)
                  {
                     Enter();
                  ...
                  if (TL.State == TouchLocationState.Pressed || TL.State == TouchLocationState.Moved)
                  {
                     ...
                     if (isDragable && DragDelayInMS < dragTime)
                     {
                        StartDrag();
                     }
                     else
            ...
            oldTouchCollection = newTouchCollection;
      ...
      public virtual void Render(SpriteBatch sb)
      {
         if (isEnabled)
         {
            ...
            {
               sb.Draw(images[clickState.ToString()], bounds.toRectangle(), null, Color.White, 0, new Vector2(0, 0), SpriteEffects.None, 0);
            }
      ...
      public virtual void Drag(TouchLocation oldTL, TouchLocation newTL)
      {
         float xMove = newTL.Position.X - oldTL.Position.X;
         float yMove = newTL.Position.Y - oldTL.Position.Y;
         if (xMove != 0 || yMove != 0)
         {
            bounds.X += newTL.Position.X - oldTL.Position.X;
            bounds.Y += newTL.Position.Y - oldTL.Position.Y;
            clickState = Enums.cState.draging;
         }
      }
